...
weedPlot_a <- weedData %>%
  ...
  ggh4x::facetted_pos_scales(y = list(scale_y_continuous(limits = c(-2, 21)),
                                      scale_y_continuous(limits = c(-30, 140)),
                                      scale_y_continuous(limits = c(-0.5, 5.75)))) +
  labs(x = "Tillage", y = "Weeds")
weedPlot_a <- weedPlot_a +
  ggplot2::stat_summary(fun.data = ggpubr::mean_se_, size = 0.125) +
  ggpubr::stat_compare_means(size = 3, label = "p.format", label.y.npc = "bottom") +
...
weedPlot_b <- weedPlot_b +
  ggplot2::stat_summary(fun.data = ggpubr::mean_se_, size = 0.125) +
  ggpubr::stat_compare_means(label = "p.format", size = 3, label.y.npc = "bottom") +
