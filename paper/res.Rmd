...
deepening the depth to hardpan by **##** cm (conv_, or ~*30-100*%) compared to no-till, 
down to 
...
```{r compactFig, fig.cap="Compaction data (a) , and (b). Significant pairwise post-hoc Wilcoxon test outcomes shown (**** p < 0.0001, *** p < 0.001, ** p < 0.01, * p < 0.05, *' p < 0.1, ' p > 0.1)"}
...
`r conv_unit(filter(infilData_TIL, TIL=="Roto")$mad, "ml_per_sec", "gal_per_min")` in
)
(Fig \@ref(fig:infilFig)a).


```{r infilFig, fig.cap="Infiltration data (a) , and (b). Significant pairwise post-hoc Wilcoxon test outcomes shown (**** p < 0.0001, *** p < 0.001, ** p < 0.01, * p < 0.05, *' p < 0.1, ' p > 0.1)"}
...
```{r weedsFig, fig.cap="Weeds data (a) , and (b). Significant pairwise post-hoc Wilcoxon test outcomes shown (**** p < 0.0001, *** p < 0.001, ** p < 0.01, * p < 0.05, *' p < 0.1, ' p > 0.1)"}
...
`r filter(weedDnData_MIX, MIX=="null")$mad` *stems per m^-2^*.
...
`r filter(weedAbnData_MIX, MIX=="wdsp")$mad` morphoscpecies taxa 
<!-- ( -->
<!-- conv_, or  -->
<!-- **%%%** -->
<!-- ) -->
...
```{r yieldFig, fig.cap="Yield data (a) from no-till and (b) tractor-till. Photo credits: Naim Edwards."}
