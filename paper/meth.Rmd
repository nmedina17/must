...
`r filter(chem, variable == "resp_mg_day") %>% .$mad %>% round(2)` mg per day 
...
All tilling was done once early in the season after one typical compost application.
...
cm *(8.75 in)* depth was measured using a `r conv_unit(6.5, "inch", "cm")` *(9.5 in)* wide aluminum cylinder, and recording the time up to 160 sec for *1 L (32 fl oz)* to pass through, representing a typical local rainfall onto ~0.10 *m^2^* (~1 *ft^2^*) of soil area ([waterdata.usgs.gov](waterdata.usgs.gov)). 
